...
contract CertChain {
    ...
    mapping(address => string) filedata;
    mapping(address => bool) isDataOwner;

    // Events
    event IssuerRegistered(address issuer, string IPFS_hash);
    event RecipientRegistered(address recipient, string IPFS_hash);
    event CertificateRegistered(address issuer, string IPFS_hash);
    event CertificateIssued(uint indexed certificate, address issuer, address recipient);
    event FiledataRegistered(address issuer, string IPFS_hash);
    ...
    function registerFiledata(string IPFS_hash) public {
        require(isIssuer[msg.sender] == true, "Issuer not registered to register a certificate");
        filedata[msg.sender] = IPFS_hash;
        emit FiledataRegistered(msg.sender, IPFS_hash);
    }
