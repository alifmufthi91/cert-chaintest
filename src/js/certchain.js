...
const CertChain = {
  ...
  registerFiledata: function (hash) {
    let self = this
    return new Promise((resolve, reject) => {
      self.instance.registerFiledata(
        hash,
        {from: window.web3.eth.accounts[0]}
      ).then(tx => {
        resolve(tx)
  ...
  eventFiledataRegistered: function (address) {
    let self = this
    return new Promise((resolve, reject) => {
      let eventFiledataRegisteredEvent = self.instance.FiledataRegistered({recipient: address}, {fromBlock: 0, toBlock: 'latest'})
      eventFiledataRegisteredEvent.get((err, res) => {
        if (err) {
          reject(err)
        } else {
          resolve(res)
