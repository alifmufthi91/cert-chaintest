<template>
    <div style="margin-top:5%; padding-bottom:50px;">
        ...
        <div v-else>
            ...
            <h1>Register filedata</h1>
            <input @change="processfile($event)" type="file"/>
            <button @click="registerFiledata">Submit</button>
...
const ipfsn = ipfsClient({ host: 'ipfs.infura.io', port: 5001, protocol: 'https' })
export default {
  name: 'issuerPage',
  data () {
    return {
  ...
  methods: {
    ...
    getCertificateIdentifier: function () {
      ...
      CertChain.getCertificateIdentifier(this.certID).then((tx) => {
        if (tx[0] !== '') {
          this.data_getCertificateIdentifier = tx[0]
    ...
    registerFiledata: function () {
      let data = this.issuer_file
      ipfsn.add(data).then(ipfsHash => {
        console.log('the result', ipfsHash[0].hash)
        CertChain.registerFiledata(ipfsHash[0].hash).then(tx => {
          console.log(tx)
          alert('File data registered')
        })
      })
    },
    processfile: function (event) {
      this.issuer_file = event.target.files[0]
      console.log(this.issuer_file)
    }
  }
}
</script>
