  ...
  methods: {
    ...
    getRecipientCertificates: function () {
      CertChain.getRecipientCertificates(this.user).then((tx) => {
        tx.forEach((value, index) => {
          console.log(value)
          CertChain.getCertificateIdentifier(value).then((data) => {
            ipfs.cat(data[0]).then((value) => {
              var parsed = JSON.parse(value)
              console.log(parsed)
              var result = parsed.cert_type
              console.log(result)
              if (value === 1) {
                this.data_getRecipientCertificates = result
              } else {
                this.data_getRecipientCertificates = this.data_getRecipientCertificates + ' ' + result
              }
            })
          })
        })
